...
OUTPUT_DIR="./results/msmarco_weighting_comparison"
MAX_QUERIES=100
BATCH_SIZE=128
WORKERS=12
# Process arguments
while [[ $# -gt 0 ]]; do
	case $1 in
		...
		--help)
			...
			echo "  --output_dir DIR      Output directory for results (default: ./results/msmarco_weighting_comparison)"
			echo "  --max_queries N       Maximum queries to process (default: 100)"
			echo "  --batch_size N        Batch size for document encoding (default: 128)"
...
python msmarco_evaluation.py \
	...
	--bm25_results "$OUTPUT_DIR/msmarco_bm25_results.tsv" \
	--bm25_index "$OUTPUT_DIR/msmarco_bm25_index.pkl" \
	$FORCE_BM25 \
	--save_runs \
	--save_metrics
